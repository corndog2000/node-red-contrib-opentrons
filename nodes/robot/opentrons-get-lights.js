...
module.exports = function(RED) {
    function OpentronsGetLightsNode(config) {
        ...
        node.on('input', async function(msg, send, done) {
            ...
            try {
                ...
                const response = await axios.get('/robot/lights', {
                    baseURL: baseURL,
                    headers: headers
                });
                
                msg.payload = response.data;
                node.status({fill: "green", shape: "dot", text: response.data.on ? "on" : "off"});
                send(msg);
                done();
            } catch (error) {
    ...
    RED.nodes.registerType("opentrons-get-lights", OpentronsGetLightsNode);
}